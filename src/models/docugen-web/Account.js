...
AccountSchema.statics.findAccount = async function (email, username) {
 const accounts = this;
 try {
  const account = await accounts.findOne({
   $or: [{ username: username }, { "user.email": email }],
  });
  return account;
 } catch (error) {
...
AccountSchema.statics.setAccountRole = async function (accountId, role) {
 const accounts = this;
 return await accounts.findOneAndUpdate({ _id: accountId }, { $set: { role: role }}, { new: true });
};

AccountSchema.statics.setAccountStatus = async function (accountId, status) {
 const accounts = this;
 return await accounts.findOneAndUpdate({ _id: accountId }, { $set: { status: status }}, { new: true });
};

AccountSchema.statics.setAccountServices = async function (accountId, services) {
 const accounts = this;
 return await accounts.findOneAndUpdate({ _id: accountId }, { $set: { services: services }}, { new: true });
};
...
AccountSchema.methods.generateAccountToken = function () {
 const account = this;
 const KEY_JWT = KEYS.backend.jwt;
 const SESSION_TIMEOUT = KEYS.backend.session_timeout;
 const payload = {
  id: account._id,
 ...
 const token = jwt.sign(payload, KEY_JWT, { expiresIn: SESSION_TIMEOUT });
 return token;
};
